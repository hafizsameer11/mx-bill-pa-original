...
class NotificationController extends GetxController {
  ...
  Future<void> fetchData() async {
    ...
    try {
      String apiUrl =
          '${AppConstants.baseUri}${AppConstants.notificationPermissionUri}';
      ...
      if (response.statusCode == 200) {
        ...
        notificationPermissions = template
            .map((template) => NotificationPermission.fromJson(template))
            .toList();
  ...
  Future<void> sendDataToApi() async {
    ...
    try {
      String apiUrl =
          '${AppConstants.baseUri}${AppConstants.notificationSubmitUri}';
      Uri uri = Uri.parse(apiUrl);
      final response = await http.post(
        uri,
        body: {
          for (int i = 0; i < userActiveEmail.length; i++)
            'email_key[$i]': userActiveEmail[i],
          for (int i = 0; i < userActiveSms.length; i++)
            'sms_key[$i]': userActiveSms[i],
          for (int i = 0; i < userActivePush.length; i++)
            'push_key[$i]': userActivePush[i],
          for (int i = 0; i < userActiveInApp.length; i++)
            'in_app_key[$i]': userActiveInApp[i],
        },
        headers: {
          'Authorization': 'Bearer $authToken',
        },
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          print(
              'Failed to change settings. Status code: ${response.statusCode}');
        }
        if (kDebugMode) {
