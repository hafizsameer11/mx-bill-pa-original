...
class SdkPaymentHandle extends GetxController {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (data['status'].toLowerCase() == "success") {
          if (billPayId != "-1") {
            Get.offNamed(PayListScreen.routeName);
          } else {
            Get.offNamed(ProfileSettingScreen.routeName);
          }
            ...
            .initPaymentSheet(
              paymentSheetParameters: SetupPaymentSheetParameters(
                  ...
                  customerEphemeralKeySecret: paymentIntentData![
                      'ephemeralKey'], //Gotten from payment intent
  ...
  createPaymentIntent(String amount, String currency) async {
    try {
      ...
      Map<String, dynamic> body = {
        'amount': (int.parse(calculateAmount(amount)) * 100).toString(),
        'currency': currency,
      };
  ...
  calculateAmount(String amount) {
    ...
    final calculatedAmount = double.parse(amount).toInt().ceil();
  ...
  Future<void> flutterwaveMakePayment({required String amount}) async {
    final Customer customer =
        Customer(email: profileController.userData!.email);

    final Flutterwave flutterwave = Flutterwave(
      context: Get.context!,
      publicKey: publicKeyFlutterWave,
      currency: AppConstants.baseCurrency.toUpperCase(),
    ...
    if (response.status == 'completed') {
      await paymentDoneRequest();
      Get.back();
    } else {
      CustomAlertDialog.showAlertDialog(
          Get.overlayContext!, "failed", "Your payment is not completed!");
    }
  }
  ...
  /* Future<void> paypalMakePayment({required String amount}) async {
    try {
      Get.to(() => PaypalCheckout(
            ...
            onError: (error) {
              CustomAlertDialog.showAlertDialog(Get.overlayContext!, "failed", "Your payment is not completed!");
              if (kDebugMode) {
