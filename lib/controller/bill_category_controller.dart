...
class BillPayController extends GetxController {
  ...
  Gateway walletPayment = Gateway(
    ...
    conventionRate: "0.00",
    sortBy:
        '0', // You can adjust the sort_by value according to your requirements
    parameters: {
      "param_key": "param_value",
      // ... other parameters
    },
    imagePath:
        "https://img.freepik.com/psd-gratis/monedero-digital-3d-nft-icon_629802-10.jpg",
  );
  ...
  void calculatePayableAmount(double amount, double charge, double rate,
      double fixedCharge, double percentageCharge) {
    var total = ((1 / rate) * (amount + charge));
    var pCharge = (total / 100) * percentageCharge;
    serviceCharges = pCharge + fixedCharge;
  ...
  Future<void> fetchBillingCategories() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final Map<String, dynamic> activeServices =
            data['message']['activeServices'];
        ...
        //   }
        // });
        billingCategories = activeServices.entries
            .map((entry) => BillingCategory.fromJson(entry.value))
            .toList();
  ...
  List<String> tcountries = [];
  List<BillingService> tserviceProviders = [];
  List<String> tserviceProviderName = [];
  List<BillingService> tbillingServices = [];
  List<BillingService> tselectableServices = [];
  BillingService? selectedSevice;
  ...
  Future<void> fetchBillForm(dynamic key) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        tbillingServices = servicesList
            .map((service) => BillingService.fromJson(service))
            .toList();

        tcountries = tbillingServices
            .map((element) => element.countryName.toString())
            .toSet()
            .toList();
  ...
  // =========================== t-start ===========================
  void tfilterServiceProvider(String country) {
    tserviceProviders.clear();
    tserviceProviders = tbillingServices
        .where((item) => item.countryName == country)
        .toList()
        .removeDuplicates((a, b) => a.code == b.code);

    update();
  }

  void tfilterSelecableServicesByServiceProvider(String code) {
    tselectableServices = tbillingServices
        .where((element) => element.code == code)
        .toList()
        .removeDuplicates((a, b) => a.type == b.type);
    update();
  }

  void tfindServiceById(String id) {
    selectedService = tselectableServices
        .firstWhere((service) => service.type.toString() == id);
    log("Amount ${selectedService!.amount.toString()}");
    update();
  }

  // filter services by country name
  void filterServicesByCountry(String country) async {
    selectableServices.clear();
    // selectableServices = billingServices.where((service) => service.countryName == country).toList();
    selectableServices = billingServices
        .where((service) => service.countryName.toString() == country)
        .toList()
        .removeDuplicates((a, b) => a.type == b.type);
  ...
  void findServiceById(dynamic id) {
    selectedService = selectableServices
        .firstWhere((service) => service.type.toString() == id);
    log("Amount ${selectedService!.amount.toString()}");
    update();
  }
  ...
  void fetchExtraResponse() {
    if (selectedService!.amount == 0 ||
        selectedService!.amount.toString() == "0") {
      amountController.text = '';
    } else {
      amountController.text = selectedService!.amount.toString();
    }
    ...
    if (selectedService!.extraResponse != null) {
  ...
  Future<void> submitBillingInfo() async {
    ...
    try {
      String apiUrl =
          '${AppConstants.baseUri}${AppConstants.billFormSubmitUri}';
      Map<String, String> request = {};

      log("ID... " + selectedService!.id.toString());
      log("Country code... " + selectedService!.country);
      log("Amount..." + (selectedExtraResponse ?? amountController.text));

      // Extract text from additional controllers and add to the request data
      request['service'] = selectedService!.id.toString();
      request['country'] = selectedService!.country;
      ...
      if (response.statusCode == 200) {
        if (data['status'].toLowerCase() == "success") {
          Get.toNamed(
              PaymentScreen.routeName); // navigate to the payment screen
          utr =
              data['message']['utr']; // saved utr to fetch payment preview data
  ...
  Future<void> tsubmitBillingInfo() async {
    final prefs = await SharedPreferences.getInstance();
    String? authToken = prefs.getString(AppConstants.token);
    isLoading = true;
    update();
  ...
  Future<void> fetchPaymentMethods(dynamic utr) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        paymentPreview!.gateways
            .insert(0, walletPayment); // inserting wallet payment manually
...
///  this is custom made extension to remove duplicated from list
extension UniqueList<T> on List<T> {
  List<T> removeDuplicates(bool Function(T, T) predicate) {
    final uniqueList = <T>[];
    for (var element in this) {
      if (!uniqueList.any((e) => predicate(e, element))) {
